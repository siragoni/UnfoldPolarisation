...
void     PolarisationHeMinuit1D(TH1* CorrectedCosTheta, TH1* CorrectedPhi, TH1* CorrectedTildePhi, Int_t SignalRangeSelectionMode = 0, Int_t FitRangeMode = 0 );
...
void UnfoldPhiReadingTree(){
  ...
  for (Int_t i=0; i<nentriesRec; i++) {
    ...
    phihelp = -999.;
      if( (pTgen > 0. && pTgen < 0.25) && (Ygen > -4. && Ygen < -2.5) ) {
          ...
          // TildePhiCSposgen2 = TildePhiHEposgen;
          // TildePhiCSneggen2 = TildePhiHEneggen;
  ...
  TFile* fileDataRawTildePhi = new TFile("RealData/TildePhiCsFrameV2.root");
  TH1F *DataTildePhi         = (TH1F*)fileDataRawTildePhi->Get("TildePhiAfterSignalExtractionErrorsH");
  TFile* fileDataRawPhi      = new TFile("RealData/PhiCsFrameV2.root");
  TH1F *DataPhi              = (TH1F*)fileDataRawPhi->Get("PhiAfterSignalExtractionErrorsH");
  TFile* fileDataRawCosTheta = new TFile("RealData/CosThetaCsFrame.root");
  ...
  TFile *SavingFile = new TFile("SavingFileDataCS.root", "RECREATE");
  PhiRecH          ->Write();
  TildePhiRecH->Write();
  PhiGenH          ->Write();
