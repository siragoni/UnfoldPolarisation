...
void UnfoldHe2D(Int_t TriggerFlag = 0, Int_t Iterations = 1){
      ...
      if( (pTgen > 0. && pTgen < 0.25) && (Ygen > -4. && Ygen < -2.5) && (TriggerSelectionFlag > 0.5) ) {
          ...
          if( (pTrec > 0. && pTrec < 0.25) && (Yrec > -4. && Yrec < -2.5) ) {
              ...
              if( (PhiHErec+TMath::Pi()) > 0. && (PhiHErec+TMath::Pi()) < 2.*TMath::Pi() ) {
                  ...
                  responsePhi[10].Fill( PhiHErec, (PhiHEgen2+TMath::Pi()) );
                  PhiGenBinsH[10]->Fill( (PhiHEgen2+TMath::Pi()) );
                  ...
                  responsePhi[10].Miss((PhiHEgen2+TMath::Pi()));
                  PhiGenBinsH[10]->Fill( (PhiHEgen2+TMath::Pi()) );
  ...
  for (Int_t iC = 4; iC < 20; iC++) {
    fileDataRaw[iC] = new TFile(Form("SignalExtractionProjection/ModulationYields/MonteCarloYieldsHe_phimodulation_%d.root", iC));
  ...
  cout << "BBBBBBBB " <<endl;
    ...
    TFile *SavingFile[24];
    for (Int_t iC = 4; iC < 20; iC++) {
      SavingFile[iC]  = new TFile(Form("SignalExtractionProjection/UnfoldingModulation/UnfoldedClosureHe_phimodulation_differentprior_%d.root", iC), "RECREATE");
      PhiRecH   ->Write();
      PhiGenH   ->Write();
