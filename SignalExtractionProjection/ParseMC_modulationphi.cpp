...
// ParseMC_modulationphi(1., 100, 0.)
    ...
    GenerTree->GetEntry(i);
      ...
      if( (pTgen > 0. && pTgen < 0.25) && (Ygen > -4. && Ygen < -2.5) ) {
        ...
        if( Flag > 0.5){
          ...
          //--------------------------------------
          InvMassHg[10][iPhiBinsg]->Fill(Mrec);
          ...
          if( (pTrec > 0. && pTrec < 0.25) && (Yrec > -4. && Yrec < -2.5) && (Flag > 0.5) ) {
                  ...
                  //--------------------------------------
                  InvMassH[10][iPhiBins]->Fill(Mrec);
  ...
  TFile *SavingFile = new TFile("SignalExtractionProjection/ModulationYields/CohHe_phimodulation.root", "RECREATE");
  for (Int_t i = 0; i < 24; i++) {
    for (Int_t j = 0; j < 24; j++) {
      InvMassH[i][j]->Write();
    }
  ...
  TFile* parsedMC = new TFile("SignalExtractionProjection/ModulationYields/CohHe_phimodulation.root");
  ...
  for (Int_t iCosThetaBins = 4; iCosThetaBins < 20; iCosThetaBins++) {
    ...
    TFile f(Form("SignalExtractionProjection/ModulationYields/MonteCarloYieldsHe_phimodulation_%d.root", iCosThetaBins),   "recreate");
