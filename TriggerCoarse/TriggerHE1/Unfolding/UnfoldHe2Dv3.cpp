...
void UnfoldHe2D(Int_t TriggerFlag = 0, Int_t Iterations = 1){
  ...
  for (Int_t iC = 4; iC < 20; iC++) {
    fileDataRaw[iC] = new TFile(Form("TriggerCoarse/TriggerHE1/RawYieldsHeV4_%d.root", iC));
  ...
  for (Int_t iC = 4; iC < 20; iC++) {
    ...
    leg_pt2[iC]->Draw();
    gPad->SaveAs(Form("TriggerCoarse/TriggerHE1/Unfolding/closureplots/refolded_data_HE_%d.pdf", iC), "recreate");
    for (Int_t j = 1; j < 30; j++) {
      if(refolded[iC]->GetBinContent(j) > 0.000000001){
  ...
  TFile *SavingFile[24];
  for (Int_t iC = 4; iC < 20; iC++) {
    SavingFile[iC]  = new TFile(Form("TriggerCoarse/TriggerHE1/Unfolding/UnfoldHeV2_%d.root", iC), "RECREATE");
    PhiRecH   ->Write();
    PhiGenH   ->Write();
