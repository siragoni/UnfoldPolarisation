...
void UnfoldData(){
  ...
  for (Int_t i = 5; i < 20; i++) {
    fData[i]  = new TFile(Form("TriggerCoarse/TriggerHE3/RawYieldsHeV3_%d.root", i));
  ...
  for (Int_t i = 5; i < 19; i++) {
    ...
    gPad->SaveAs(Form("TriggerCoarse/TriggerHE3/Convergence/unfolded-data-%d.pdf", i), "recreate");
  ...
  for (Int_t i = 5; i < 19; i++) {
    ...
    latex5->DrawLatex(0.31,0.82,Form("Iterations %d", N));
    latex5->DrawLatex(0.31,0.74,Form("This thesis, Slice in cos#theta = %d", i));
    gPad->SaveAs(Form("TriggerCoarse/TriggerHE3/Convergence/unfolded-data-relative-uncertainties-%d.pdf", i), "recreate");
