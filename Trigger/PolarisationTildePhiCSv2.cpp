...
void fitJPsiTemplateMC(const int selectionFlag = 0){
  ...
  // }
  fCohJpsiToMu        = (TH1F*)listingsMC[0]->FindObject(Form("fInvariantMassDistributionOnlyTildePhiCsFrameTwentyfiveBinsHv2_%d", selectionFlag));
  // fCohPsi2sToMu       = (TH1F*)listingsMC[1]->FindObject(Form("fInvariantMassDistributionOnlyTildePhiCsFrameTwentyfiveBinsH_%d", selectionFlag));
  ...
  // fTwoGammaToMuMedium = (TH1F*)listingsMC[6]->FindObject(Form("fInvariantMassDistributionOnlyTildePhiCsFrameTwentyfiveBinsH_%d", selectionFlag));
...
void fitJPsiTemplate(const int selectionFlag, Int_t SignalRangeModeFlag = 0){
  ...
  fInvariantMassDistributionH = (TH1F*)listings->FindObject( Form("fInvariantMassDistributionOnlyTildePhiCsFrameTwentyfiveBinsHv2_%d", selectionFlag) );
  ...
  // latex->DrawLatex(0.55,0.78,Form("#color[2]{%.3f < #tilde{#phi} < %.3f}", 0 + (Double_t)selectionFlag * 0.2512, 0 + ((Double_t)selectionFlag + 1.00) * 0.2512));
  latex->DrawLatex(0.55,0.78,Form("#color[2]{%.3f < #tilde{#phi} < %.3f}", 2.*TMath::Pi()*(Double_t)selectionFlag / 25., 2.*TMath::Pi()*((Double_t)selectionFlag + 1.00) / 25.));
  ...
  // gPad->SaveAs(Form("pngResults/TildePhiCs_%d.png", selectionFlag), "recreate");
  TDatime d;
  if      ( SignalRangeModeFlag == 0 ) { gPad->SaveAs(Form("pngResults/%d-%2.2d-%2.2d/TildePhiCSv2/TildePhiCs_%d.png",        d.GetYear(), d.GetMonth(), d.GetDay(), selectionFlag), "recreate"); }
  else if ( SignalRangeModeFlag == 1 ) { gPad->SaveAs(Form("pngResults/%d-%2.2d-%2.2d/TildePhiCSv2/TildePhiCs_%d_Range1.png", d.GetYear(), d.GetMonth(), d.GetDay(), selectionFlag), "recreate"); }
  else if ( SignalRangeModeFlag == 2 ) { gPad->SaveAs(Form("pngResults/%d-%2.2d-%2.2d/TildePhiCSv2/TildePhiCs_%d_Range2.png", d.GetYear(), d.GetMonth(), d.GetDay(), selectionFlag), "recreate"); }
  else if ( SignalRangeModeFlag == 3 ) { gPad->SaveAs(Form("pngResults/%d-%2.2d-%2.2d/TildePhiCSv2/TildePhiCs_%d_Range3.png", d.GetYear(), d.GetMonth(), d.GetDay(), selectionFlag), "recreate"); }
  else if ( SignalRangeModeFlag == 4 ) { gPad->SaveAs(Form("pngResults/%d-%2.2d-%2.2d/TildePhiCSv2/TildePhiCs_%d_Range4.png", d.GetYear(), d.GetMonth(), d.GetDay(), selectionFlag), "recreate"); }
  else if ( SignalRangeModeFlag == 5 ) { gPad->SaveAs(Form("pngResults/%d-%2.2d-%2.2d/TildePhiCSv2/TildePhiCs_%d_Range5.png", d.GetYear(), d.GetMonth(), d.GetDay(), selectionFlag), "recreate"); }
  else                                 { gPad->SaveAs(Form("pngResults/%d-%2.2d-%2.2d/TildePhiCSv2/TildePhiCs_%d.png",        d.GetYear(), d.GetMonth(), d.GetDay(), selectionFlag), "recreate"); }

  // if      ( SignalRangeModeFlag == 0 ) { gPad->SaveAs(Form("pngResults/TildePhiCs_%d.png",        selectionFlag), "recreate"); }
  // else if ( SignalRangeModeFlag == 1 ) { gPad->SaveAs(Form("pngResults/TildePhiCs_%d_Range1.png", selectionFlag), "recreate"); }
  // else if ( SignalRangeModeFlag == 2 ) { gPad->SaveAs(Form("pngResults/TildePhiCs_%d_Range2.png", selectionFlag), "recreate"); }
  // else if ( SignalRangeModeFlag == 3 ) { gPad->SaveAs(Form("pngResults/TildePhiCs_%d_Range3.png", selectionFlag), "recreate"); }
  // else if ( SignalRangeModeFlag == 4 ) { gPad->SaveAs(Form("pngResults/TildePhiCs_%d_Range4.png", selectionFlag), "recreate"); }
  // else if ( SignalRangeModeFlag == 5 ) { gPad->SaveAs(Form("pngResults/TildePhiCs_%d_Range5.png", selectionFlag), "recreate"); }
  // else                                 { gPad->SaveAs(Form("pngResults/TildePhiCs_%d.png",        selectionFlag), "recreate"); }
 ...
 * - 1: ehm
 */
void CreateCosThetaTh1(const char* AnalysisName, Int_t TriggerFlag, Int_t SignalRangeMode = 0){
  // fileMC[0] = new TFile("MCtrainResults/2019-09-17/kCohJpsiToMu/AnalysisResults.root");
  fileMC[0] = new TFile("AnalysisResultsLHC18l716b2_nophoton_phi.root");
  ...
  dir->GetObject(Form("MyOutputContainer%d", TriggerFlag), listings);
  ...
  for (size_t iCosThetaBins = 0; iCosThetaBins < 25; iCosThetaBins++) {
    // for (size_t iPhiBins = 0; iPhiBins < 10; iPhiBins++) {
      ...
      // TildePhiAfterSignalExtractionH->Fill( 0 + 0.1256 + (Double_t)iCosThetaBins * 0.2512,
      //                                       JPsiPeakValue
      //                                       );
      // TildePhiGammaGammaH->Fill( 0 + 0.1256  + (Double_t)iCosThetaBins * 0.2512,
      //                            BkgValue
      //                            );
      //
      // TildePhiAfterSignalExtractionErrorsH->Fill( 0 + 0.1256  + (Double_t)iCosThetaBins * 0.2512,
      //                                             JPsiPeakValue
      //                                             );
      // TildePhiGammaGammaErrorsH->Fill(  0 + 0.1256  + (Double_t)iCosThetaBins * 0.2512,
      //                                   BkgValue
      //                                   );
      // TildePhiAfterSignalExtractionErrorsH->SetBinError(  iCosThetaBins + 1 ,
      //                                                     JPsiPeakValueErr
      //                                                     );
      TildePhiAfterSignalExtractionH->Fill(  TildePhiAfterSignalExtractionH->GetBinCenter( iCosThetaBins + 1 ), JPsiPeakValue );
      TildePhiGammaGammaH           ->Fill(  TildePhiGammaGammaH           ->GetBinCenter( iCosThetaBins + 1 ), BkgValue      );

      TildePhiAfterSignalExtractionErrorsH->Fill(  TildePhiAfterSignalExtractionErrorsH->GetBinCenter( iCosThetaBins + 1 ), JPsiPeakValue );
      TildePhiGammaGammaErrorsH           ->Fill(  TildePhiGammaGammaErrorsH           ->GetBinCenter( iCosThetaBins + 1 ), BkgValue      );
      TildePhiGammaGammaErrorsH           ->SetBinError(   iCosThetaBins + 1 , BkgValueError    );
      TildePhiAfterSignalExtractionErrorsH->SetBinError(   iCosThetaBins + 1 , JPsiPeakValueErr );

    // }
  }

  // TFile f("pngResults/TildePhiCsFrame.root", "recreate");
  if      ( SignalRangeMode == 0 ) {
    TDatime d;
    TFile f(Form("TriggerUncert/PolTrigger%d/TildePhiCSv2.root.root", TriggerFlag), "recreate");
    // TFile f("pngResults/TildePhiCsFrame.root",   "recreate");
    ...
    TFile f("pngResults/TildePhiCsFrame_1.root", "recreate");
    TildePhiAfterSignalExtractionH      ->Write();
    TildePhiGammaGammaH                 ->Write();
    TildePhiAfterSignalExtractionErrorsH->Write();
    TildePhiGammaGammaErrorsH           ->Write();
    ...
    TFile f("pngResults/TildePhiCsFrame_2.root", "recreate");
    TildePhiAfterSignalExtractionH      ->Write();
    TildePhiGammaGammaH                 ->Write();
    TildePhiAfterSignalExtractionErrorsH->Write();
    TildePhiGammaGammaErrorsH           ->Write();
    ...
    TFile f("pngResults/TildePhiCsFrame_3.root", "recreate");
    TildePhiAfterSignalExtractionH      ->Write();
    TildePhiGammaGammaH                 ->Write();
    TildePhiAfterSignalExtractionErrorsH->Write();
    TildePhiGammaGammaErrorsH           ->Write();
    ...
    TFile f("pngResults/TildePhiCsFrame_4.root", "recreate");
    TildePhiAfterSignalExtractionH      ->Write();
    TildePhiGammaGammaH                 ->Write();
    TildePhiAfterSignalExtractionErrorsH->Write();
    TildePhiGammaGammaErrorsH           ->Write();
    ...
    TFile f("pngResults/TildePhiCsFrame_5.root", "recreate");
    TildePhiAfterSignalExtractionH      ->Write();
    TildePhiGammaGammaH                 ->Write();
    TildePhiAfterSignalExtractionErrorsH->Write();
    TildePhiGammaGammaErrorsH           ->Write();
