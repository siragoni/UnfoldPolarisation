...
void fitJPsiTemplateMC(const int selectionFlag = 0){
  ...
  // }
  fCohJpsiToMu        = (TH1F*)listingsMC[0]->FindObject(Form("fInvariantMassDistributionOnlyPhiHeFrameTwentyfiveBinsHv2_%d", selectionFlag));
  // fCohPsi2sToMu       = (TH1F*)listingsMC[1]->FindObject(Form("fInvariantMassDistributionOnlyPhiHeFrameTwentyfiveBinsH_%d", selectionFlag));
  ...
  // fTwoGammaToMuMedium = (TH1F*)listingsMC[6]->FindObject(Form("fInvariantMassDistributionOnlyPhiHeFrameTwentyfiveBinsH_%d", selectionFlag));
...
void fitJPsiTemplate(const int selectionFlag, Int_t SignalRangeModeFlag = 0){
  ...
  fInvariantMassDistributionH = (TH1F*)listings->FindObject( Form("fInvariantMassDistributionOnlyPhiHeFrameTwentyfiveBinsHv2_%d", selectionFlag) );
  ...
  fFitInvMass->SetParameter(15, 1);
  if( selectionFlag == 17 ){
    fFitInvMass->SetParameter(16, 90);
    fFitInvMass->SetParameter(17, 7000);
  } else {
    fFitInvMass->SetParameter(16, 1);
    fFitInvMass->SetParameter(17, 1);
  }
  // fFitInvMass->FixParameter(17, 7108);
  // if(selectionFlag == 19) {
  ...
  // if( selectionFlag == 17 ){
  //   fFitInvMass->SetParLimits(15, 0.001, 9999999999);
  //   fFitInvMass->SetParLimits(16, 50, 150);
  //   fFitInvMass->SetParLimits(17, 5000, 8000);
  // } else {
    fFitInvMass->SetParLimits(15, 0.001, 9999999999);
    fFitInvMass->SetParLimits(16, 0.001, 9999999999);
    fFitInvMass->SetParLimits(17, 0.001, 9999999999);
  ...
  TDatime d;
  if      ( SignalRangeModeFlag == 0 ) { gPad->SaveAs(Form("pngResults/%d-%2.2d-%2.2d/PhiHEv2/PhiHe_%d.png",        d.GetYear(), d.GetMonth(), d.GetDay(), selectionFlag), "recreate"); }
  else if ( SignalRangeModeFlag == 1 ) { gPad->SaveAs(Form("pngResults/%d-%2.2d-%2.2d/PhiHEv2/PhiHe_%d_Range1.png", d.GetYear(), d.GetMonth(), d.GetDay(), selectionFlag), "recreate"); }
  else if ( SignalRangeModeFlag == 2 ) { gPad->SaveAs(Form("pngResults/%d-%2.2d-%2.2d/PhiHEv2/PhiHe_%d_Range2.png", d.GetYear(), d.GetMonth(), d.GetDay(), selectionFlag), "recreate"); }
  else if ( SignalRangeModeFlag == 3 ) { gPad->SaveAs(Form("pngResults/%d-%2.2d-%2.2d/PhiHEv2/PhiHe_%d_Range3.png", d.GetYear(), d.GetMonth(), d.GetDay(), selectionFlag), "recreate"); }
  else if ( SignalRangeModeFlag == 4 ) { gPad->SaveAs(Form("pngResults/%d-%2.2d-%2.2d/PhiHEv2/PhiHe_%d_Range4.png", d.GetYear(), d.GetMonth(), d.GetDay(), selectionFlag), "recreate"); }
  else if ( SignalRangeModeFlag == 5 ) { gPad->SaveAs(Form("pngResults/%d-%2.2d-%2.2d/PhiHEv2/PhiHe_%d_Range5.png", d.GetYear(), d.GetMonth(), d.GetDay(), selectionFlag), "recreate"); }
  else                                 { gPad->SaveAs(Form("pngResults/%d-%2.2d-%2.2d/PhiHEv2/PhiHe_%d.png",        d.GetYear(), d.GetMonth(), d.GetDay(), selectionFlag), "recreate"); }
...
void CreateCosThetaTh1(const char* AnalysisName, Int_t TriggerFlag, Int_t SignalRangeMode = 0){
  ...
  // for (size_t iCosThetaBins = 17; iCosThetaBins < 18; iCosThetaBins++) {
  for (size_t iCosThetaBins = 0; iCosThetaBins < 25; iCosThetaBins++) {
    // for (size_t iPhiBins = 0; iPhiBins < 10; iPhiBins++) {
      ...
      // PhiAfterSignalExtractionH->Fill(  -3.14 + 0.1256 + (Double_t)iCosThetaBins * 0.2512, JPsiPeakValue );
      // PhiGammaGammaH->Fill( -3.14 + 0.1256  + (Double_t)iCosThetaBins * 0.2512, BkgValue );
      //
      // PhiAfterSignalExtractionErrorsH->Fill(  -3.14 + 0.1256  + (Double_t)iCosThetaBins * 0.2512, JPsiPeakValue );
      // PhiGammaGammaErrorsH->Fill(   -3.14 + 0.1256  + (Double_t)iCosThetaBins * 0.2512, BkgValue );
  ...
  if      ( SignalRangeMode == 0 ) {
    TDatime d;
    // if      ( SignalRangeModeFlag == 0 ) { gPad->SaveAs(Form("pngResults/%d-%2.2d-%2.2d/PhiHEv2/PhiHe_%d.png",        d.GetYear(), d.GetMonth(), d.GetDay(), selectionFlag), "recreate"); }
    TFile f(Form("TriggerUncert/PolTrigger%d/PhiHEv2.root", TriggerFlag)  , "recreate");
    PhiAfterSignalExtractionH      ->Write();
    PhiGammaGammaH                 ->Write();
    PhiAfterSignalExtractionErrorsH->Write();
    PhiGammaGammaErrorsH           ->Write();
