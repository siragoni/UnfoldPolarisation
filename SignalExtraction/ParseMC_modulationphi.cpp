...
// ParseMC_modulationphi(1., 100, 0.)
  ...
  for (Int_t i=0; i<nentriesRec; i++) {
    ...
    GenerTree->GetEntry(i);
      ...
      if( (pTgen > 0. && pTgen < 0.25) && (Ygen > -4. && Ygen < -2.5) ) {
        ...
        Double_t TraslatedPhiGen = PhiHEgen2*24./(2.*TMath::Pi());
        Double_t iPhiBins2 = -1;
        Double_t RemainderPhi = 0.;
        RemainderPhi = modf(TraslatedPhiGen, &iPhiBins2);
        Int_t iPhiBins = (Int_t)  iPhiBins2;
        ...
        // Int_t biny2 = generatedlimit->GetYaxis()->FindBin(CosThetaHEgen2);
        // Int_t binx2 = generatedlimit->GetXaxis()->FindBin(PhiHEgen2);
        ...
        // if ( generatedlimit->GetBinContent(binx2,biny2) < (Threshold * Model->Eval(PhiHEgen2, CosThetaHEgen2))  ){
        // Double_t parameter[4] = {LTheta, LPhi, 0, Threshold};
        // Double_t xvalues[2] = {PhiHEgen2, CosThetaHEgen2};
        // if ( generatedlimit->GetBinContent(binx2,biny2) < (IntegralFunction(xvalues, parameter))  ){
  ...
  TFile *SavingFile = new TFile("SignalExtraction/CohHe_phimodulation.root", "RECREATE");
  ...
  TFile* parsedMC = new TFile("SignalExtraction/CohHe_phimodulation.root");
  ...
  for (Int_t iCosThetaBins = 4; iCosThetaBins < 20; iCosThetaBins++) {
    ...
    TFile f(Form("SignalExtraction/MonteCarloYieldsHe_phimodulation_%d.root", iCosThetaBins),   "recreate");
  ...
  TFile* SimpleClosure = new TFile("SignalExtraction/SimpleClosure_phimodulation.root", "RECREATE");
