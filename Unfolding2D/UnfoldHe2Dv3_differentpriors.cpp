...
Double_t weight_prior2D(Double_t Phi, Double_t CosTheta){
  Double_t returnValue = (1.+(2.*0.01/3.95)*TMath::Cos(2.*Phi))/(2.*TMath::Pi());
  Double_t CosSquaredTheta      = CosTheta * CosTheta;
  // Double_t partialTheta         = 1 + 0.95 * CosSquaredTheta;
  Double_t partialTheta         = 1 + 0.6 * CosSquaredTheta;
  // Double_t partialTheta         = 1 + 1.18 * CosSquaredTheta;

  Double_t CosOfTwoPhi          = TMath::Cos( 2 * Phi );
  Double_t SinSquaredTheta      = 1 - CosSquaredTheta;
  // Double_t partialPhi           = 0.05 * SinSquaredTheta * CosOfTwoPhi;
  Double_t partialPhi           = 0.0 * SinSquaredTheta * CosOfTwoPhi;
  // Double_t partialPhi           = 0.04 * SinSquaredTheta * CosOfTwoPhi;
  ...
  // Double_t partialMix           = 0.15 * SinOfTwoTheta * CosPhi;
  Double_t partialMix           = -0.004 * SinOfTwoTheta * CosPhi;
  // Double_t partialMix           = 0.11 * SinOfTwoTheta * CosPhi;


  Double_t sumOfTheSubFits      = partialTheta + partialPhi + partialMix;
  // Double_t FinalResult          = 1. * sumOfTheSubFits / ( 3.95  );
  // Double_t FinalResult          = 1. * sumOfTheSubFits / ( 3.6 );
  // return   (1./FinalResult);
  return   FinalResult;

}

Double_t weight_prior(Double_t Phi){
  Double_t returnValue = (1.+(2.*0.01/3.95)*TMath::Cos(2.*Phi))/(2.*TMath::Pi());
  return returnValue;
}

/* In this case I renormalise everything
...
void UnfoldHe2D(Int_t TriggerFlag = 0, Int_t Iterations = 1){
              ...
              if( (PhiHErec+TMath::Pi()) > 0. && (PhiHErec+TMath::Pi()) < 2.*TMath::Pi() ){
                  ...
                  for (Int_t iCounter = 0; iCounter < 24; iCounter++) {
                    ...
                    if(  ( CosThetaHErec > (-1. + ((Double_t) iCounter)*(0.08+0.01/3.)) && CosThetaHErec < (-1. + (((Double_t) iCounter)+1.)*(0.08+0.01/3.) )) )
                    {
                      ...
                      // responsePhi[iCounter].Fill( PhiHErec, (PhiHEgen2+TMath::Pi()), weight_prior2D(PhiHEgen2+TMath::Pi(), CosThetaHEgen2) );
                      responsePhi[iCounter].Fill( PhiHErec, (PhiHEgen2+TMath::Pi()), weight_prior2D(PhiHEgen2+TMath::Pi(), CosThetaHErec) );
                      // responsePhi[iCounter].Fill( PhiHErec, (PhiHEgen2+TMath::Pi()), weight_prior(PhiHEgen2+TMath::Pi()));
                  ...
                  for (Int_t iCounter = 0; iCounter < 24; iCounter++) {
                    if(  ( CosThetaHEgen2 > (-1. + ((Double_t) iCounter)*(0.08+0.01/3.)) && CosThetaHEgen2 < (-1. + (((Double_t) iCounter)+1.)*(0.08+0.01/3.) )) )
                    {
                      ...
                      responsePhi[iCounter].Miss((PhiHEgen2+TMath::Pi()), weight_prior2D(PhiHEgen2+TMath::Pi(), CosThetaHEgen2));
                      // responsePhi[iCounter].Miss((PhiHEgen2+TMath::Pi()), weight_prior(PhiHEgen2+TMath::Pi()));
  ...
  for (Int_t iC = 4; iC < 20; iC++) {
    fileDataRaw[iC] = new TFile(Form("SignalExtraction/RawYieldsHeV3_%d.root", iC));
  ...
  for (Int_t i = 4; i < 20; i++) {
    ...
    for(Int_t j = 1; j < 30; j++){
      // histo[i]->SetBinContent(j, Data[i]->GetBinContent(j)*(0.08+0.01/3.)*((Double_t) N) *2.*TMath::Pi() );
      // histo[i]->SetBinError(  j, Data[i]->GetBinError(j)  *(0.08+0.01/3.)*((Double_t) N) *2.*TMath::Pi() );
