...
struct MyIntegFunc
{
    ...
    Double_t IntegralFunction = fFunc->Integral(
   ...
   TF2 *fFunc; // pointer to the integral function
   double param[4];
};
...
void FcnForMinimisationWithBins(Int_t & /*nPar*/, Double_t * /*grad*/ , Double_t &fval, Double_t *p, Int_t /*iflag */  )
{
  ...
  for ( Int_t i = 0; i < n; ++i ) {
    ...
    Double_t TraslatedCosThetaGen = 0.5*(coordsX[i] + 1.)*24.;
    ...
    Double_t TraslatedPhiGen = coordsY[i]*M/(2.*TMath::Pi());
...
void DrawRefolded()
{
  ...
  // UnlikeSignDimuon->GetXaxis()->SetRangeUser(0. , 2.*T);
  ...
  latex5->SetNDC();
  latex5->DrawLatex(0.31,0.94,"ALICE LHC18l7, PbPb #sqrt{s_{NN}} = 5.02 TeV");
  latex5->DrawLatex(0.31,0.74,"This thesis, 100 iterations");

  gPad->SaveAs("Unfolding2D/residual-refolded-modulationphi.pdf", "recreate");
}
//_____________________________________________________________________________
/* - Fit function for the helicity case. It is basically a parabolic fit...
   -
