...
Double_t weight_prior(Double_t Phi){
  Double_t returnValue = (1.+(2./3.)*TMath::Cos(2.*Phi))/(2.*TMath::Pi());
  return returnValue;
}
Double_t weight_fake(Double_t Phi){
  Double_t returnValue = (1.)/(2.*TMath::Pi());
  return returnValue;
}
Double_t weight_prior2D(Double_t Phi, Double_t CosTheta){
  Double_t returnValue = (1.+(2./3.)*TMath::Cos(2.*Phi))/(2.*TMath::Pi());
  Double_t CosSquaredTheta      = CosTheta * CosTheta;
  Double_t partialTheta         = 1 + 0. * CosSquaredTheta;

  Double_t CosOfTwoPhi          = TMath::Cos( 2 * Phi );
  Double_t SinSquaredTheta      = 1 - CosSquaredTheta;
  Double_t partialPhi           = 1. * SinSquaredTheta * CosOfTwoPhi;
  ...
  Double_t partialMix           = 0. * SinOfTwoTheta * CosPhi;


  Double_t sumOfTheSubFits      = partialTheta + partialPhi + partialMix;
  Double_t FinalResult          = 1. * sumOfTheSubFits / ( 3  );
  return   FinalResult;
...
void UnfoldHe2D(Int_t TriggerFlag = 0, Int_t Iterations = 1){
          ...
          if( (pTrec > 0. && pTrec < 0.25) && (Yrec > -4. && Yrec < -2.5) ) {
              ...
              if( (PhiHErec+TMath::Pi()) > 0. && (PhiHErec+TMath::Pi()) < 2.*TMath::Pi() ) {
                  ...
                  for (Int_t iCounter = 0; iCounter < 24; iCounter++) {
                    ...
                    {
                      // responsePhi[iCounter].Fill( PhiHErec, (PhiHEgen2+TMath::Pi()), weight_fake(PhiHEgen2+TMath::Pi()) );
                  ...
                  for (Int_t iCounter = 0; iCounter < 24; iCounter++) {
                    if(  ( CosThetaHEgen2 > (-1. + ((Double_t) iCounter)*(0.08+0.01/3.)) &&
                           CosThetaHEgen2 < (-1. + (((Double_t) iCounter)+1.)*(0.08+0.01/3.) )) )
                    {
                      // responsePhi[iCounter].Miss((PhiHEgen2+TMath::Pi()), weight_fake(PhiHEgen2+TMath::Pi()));
  ...
  for (Int_t iC = 4; iC < 20; iC++) {
    ...
    unfolded[iC]  = (TH1D*) unfold[iC].Hreco(RooUnfold::kErrors);
    unfolded1[iC] = (TH1D*) unfold1[iC].Hreco(RooUnfold::kErrors);
    unfolded2[iC] = (TH1D*) unfold2[iC].Hreco(RooUnfold::kErrors);
    unfolded3[iC] = (TH1D*) unfold3[iC].Hreco(RooUnfold::kErrors);
  }
  ...
  TFile *SavingFileSimpleUnfold;
    ...
    for (Int_t j = 1; j < 30; j++) {
      if(refolded[iC]->GetBinContent(j) > 0.000000001){
        ...
        // histo9->SetBinContent(Counter, histo[iC]->GetBinContent(j)*histo[iC]->GetBinContent(j)*(refolded[iC]->GetBinContent(j)-1.)*(refolded[iC]->GetBinContent(j)-1.)/(refolded[iC]->GetBinError(j)));
        Counter++;
      }
    }
