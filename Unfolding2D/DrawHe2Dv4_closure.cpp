...
void FcnForMinimisationWithBins(Int_t & /*nPar*/, Double_t * /*grad*/ , Double_t &fval, Double_t *p, Int_t /*iflag */  )
{
  ...
  for ( Int_t i = 0; i < n; ++i ) {
    ...
    if ( values[i] != 0 ) {
      ...
      tmp = ( values[i] - IntegralFunction(y,p)) / (1.*errors[i]);
...
void PolarisationHeMinuit2D(Int_t FitRangeMode = 0, Int_t Iterations = 1, Int_t StripeMode = 0 ){
  ...
  for (Int_t i = 4; i < 20; i++) {
    ...
    CosThetaGenH = (TH1F*) f[i]->Get("CosThetaGenH");
  ...
  new TCanvas;
  BeautifyPad();
  BeautifyHisto(CosThetaGenH);
  CosThetaGenH->GetXaxis()->SetTitle("#cos#theta");
  CosThetaGenH->GetYaxis()->SetTitle("Counts");
  gPad->SaveAs("Unfolding2D/plotsThesis/generated-costheta.pdf", "recreate");
  ...
  UnlikeSignDimuon->GetYaxis()->SetTitle("(Refolded - REC)/REC [a.u.]");
  // UnlikeSignDimuon->GetYaxis()->SetRangeUser(0.99,1.01);
  // UnlikeSignDimuon->GetXaxis()->SetRangeUser(-0.5 , 229.5);
  // UnlikeSignDimuon->GetYaxis()->SetRangeUser(UnlikeSignDimuon->GetMaximum()*(0.9)*(0.000000000000001),UnlikeSignDimuon->GetMaximum()*(1.2)*(0.000000000000001));
  ...
  gPad->SaveAs(Form("Unfolding2D/plotsThesis/residual-refolded-%d.pdf", Iterations), "recreate");
  ...
  for (Int_t iCosThetaBins = 7; iCosThetaBins < 18; iCosThetaBins++) {
  ...
  histo2->SetTitle( "; #it{#varphi} ;cos(#it{#theta})" );
    ...
    gPad->SaveAs(Form("Unfolding2D/plotsThesis/Inverted2Dmaps-%d.pdf", Iterations), "recreate");
    ...
    gPad->SaveAs(Form("Unfolding2D/plotsThesis/3D-%d.pdf", Iterations), "recreate");
  ...
  gPad->SaveAs("Unfolding2D/plotsThesis/residuals-closure.pdf", "recreate");
  ...
  //==========================
  // Plot fit to single stripe
  //--------------------------
  if ( StripeMode != 0 ) {
    new TCanvas;
    BeautifyPad();
    BeautifyHisto(h[StripeMode]);
    h[StripeMode]->GetXaxis()->SetTitle("#varphi");
    h[StripeMode]->GetYaxis()->SetTitle("Unfolded data [a.u.]");
    h[StripeMode]->GetYaxis()->SetRangeUser(h[StripeMode]->GetMaximum()*(0.69),h[StripeMode]->GetMaximum()*(1.003));
    // resDistrib->GetXaxis()->SetRangeUser(-0.5, 215.5);
    h[StripeMode]->Draw("");
    TH1F* modelhis3 = new TH1F("modelhis3","modelhis3",h[StripeMode]->GetNbinsX(), 0., 2.*TMath::Pi());
    for (Int_t i = 0; i < h[StripeMode]->GetNbinsX(); i++) {
      Double_t xn2[2] = {(Double_t)i, (Double_t)StripeMode};
      Double_t yn2[4] = {LambdaTheta,LambdaPhi,LambdaThetaPhi,Normalisation};
      modelhis3->SetBinContent(i+1, IntegralFunction(xn2, yn2));
    }
    modelhis3->SetLineColor(kBlue);
    modelhis3->SetLineWidth(2);
    modelhis3->Draw("same");
    TLatex* latexD = new TLatex();
    latexD->SetTextSize(0.045);
    latexD->SetTextFont(42);
    ...
    latexD->SetNDC();
    latexD->DrawLatex(0.31,0.94,"This thesis, Pb-Pb #sqrt{#it{s}_{NN}} = 5.02 TeV");
    latexD->DrawLatex(0.31,0.8,Form("Stripe %d", StripeMode));
    gPad->SaveAs(Form("Unfolding2D/plotsThesis/stripe-closure-%d.pdf", StripeMode), "recreate");
