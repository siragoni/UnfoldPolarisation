...
void UnfoldHe2D(Int_t TriggerFlag = 0, Int_t Iterations = 1, Double_t LPhi = 1.){
  ...
  for (Int_t iC = 4; iC < 20; iC++) {
    fileDataRaw[iC] = new TFile(Form("ScanModulation/LPhi1/ModulationYields/MonteCarloYieldsHe_phimodulation_%d.root", iC));
  ...
  cout << "BBBBBBBB " <<endl;
    ...
    TFile *SavingFile[24];
    for (Int_t iC = 4; iC < 20; iC++) {
      SavingFile[iC]  = new TFile(Form("ScanModulation/LPhi1/UnfoldingModulation/UnfoldedClosureHe_phimodulation_differentprior_%d.root", iC), "RECREATE");
      PhiRecH   ->Write();
      PhiGenH   ->Write();
