...
Double_t weight_prior2D(Double_t Phi, Double_t CosTheta, Double_t LPhiValue){
  ...
  Double_t partialPhi           = LPhiValue * SinSquaredTheta * CosOfTwoPhi;
...
void UnfoldHe2D(Int_t TriggerFlag = 0, Int_t Iterations = 1, Double_t LPhi = 1.){
  //
  // #ifdef __CINT__
              ...
              if( (PhiHErec+TMath::Pi()) > 0. && (PhiHErec+TMath::Pi()) < 2.*TMath::Pi() ) {
                  ...
                  for (Int_t iCounter = 0; iCounter < 24; iCounter++) {
                           ...
                           CosThetaHEgen2 < (-1. + (((Double_t) iCounter)+1.)*(0.08+0.01/3.) )) )
                    {
                      ...
                      responsePhi[iCounter].Fill( PhiHErec, (PhiHEgen2+TMath::Pi()), weight_prior2D(PhiHEgen2+TMath::Pi(), CosThetaHEgen2, LPhi) );
                  ...
                  for (Int_t iCounter = 0; iCounter < 24; iCounter++) {
                           ...
                           CosThetaHEgen2 < (-1. + (((Double_t) iCounter)+1.)*(0.08+0.01/3.) )) )
                    {
                      ...
                      responsePhi[iCounter].Miss((PhiHEgen2+TMath::Pi()), weight_prior2D(PhiHEgen2+TMath::Pi(), CosThetaHEgen2, LPhi));
                      PhiGenBinsH[iCounter]->Fill( (PhiHEgen2+TMath::Pi()) );
  ...
  for (Int_t iC = 4; iC < 20; iC++) {
    fileDataRaw[iC] = new TFile(Form("ScanModulation/LPhi02/ModulationYields/MonteCarloYieldsHe_phimodulation_%d.root", iC));
  ...
  cout << "BBBBBBBB " <<endl;
    ...
    TFile *SavingFile[24];
    for (Int_t iC = 4; iC < 20; iC++) {
      SavingFile[iC]  = new TFile(Form("ScanModulation/LPhi02/UnfoldingModulation/UnfoldedClosureHe_phimodulation_differentprior_%d.root", iC), "RECREATE");
      PhiRecH   ->Write();
      PhiGenH   ->Write();
