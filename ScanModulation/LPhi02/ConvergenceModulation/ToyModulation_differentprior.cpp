...
void ToyModulation(Int_t Stripe = 14){
  ...
  for (Int_t i = 5; i < 20; i++) {
    fData[i]  = new TFile(Form("ScanModulation/LPhi02/ModulationYields/MonteCarloYieldsHe_phimodulation_%d.root", i));
  ...
  gPad->SaveAs(Form("ScanModulation/LPhi02/ConvergenceModulation/chi2-unfold-errors-modulation-different-prior-%d.pdf", Stripe), "recreate");
  new TCanvas;
  BeautifyPad();
  ...
  // latex5->DrawLatex(0.31,0.74,"This thesis");
  latex5->DrawLatex(0.31,0.74,Form("This thesis, Slice in cos#theta = %d", Stripe));
  gPad->SaveAs(Form("ScanModulation/LPhi02/ConvergenceModulation/chi2-generated-errors-modulation-different-prior-%d.pdf", Stripe), "recreate");
  ...
  latex5->DrawLatex(0.31,0.94,"Pb-Pb #sqrt{#it{s}_{NN}} = 5.02 TeV");
  latex5->DrawLatex(0.31,0.74,Form("This thesis, Slice in cos#theta = %d", Stripe));
  gPad->SaveAs(Form("ScanModulation/LPhi02/ConvergenceModulation/average-rho-data-prior-%d.pdf", Stripe), "recreate");
  averagerhoH->GetXaxis()->SetRangeUser(-0.5, 49.5);
  gPad->SaveAs(Form("ScanModulation/LPhi02/ConvergenceModulation/average-rho-data-prior-zoom-%d.pdf", Stripe), "recreate");
  averagerhoH->GetXaxis()->SetRangeUser(-0.5, 1999.5);
  gPad->SaveAs(Form("ScanModulation/LPhi02/ConvergenceModulation/average-rho-data-prior-%d.pdf", Stripe), "recreate");
