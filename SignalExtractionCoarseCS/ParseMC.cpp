...
void ParseMC(){
  ...
  GenerTree->SetBranchAddress("fMCCosThetaCS",&CosThetaHEgen);
  ...
  GenerTree->SetBranchAddress("fMCPhiCS",&PhiHEgen);
  ...
  TFile *SavingFile = new TFile("SignalExtractionCoarseCS/ClosureYields/CohHe.root", "RECREATE");
  PhiRecH->Write();
  PhiRecHH->Write();
  PhiGenH->Write();
  PhiGenHH->Write();
  ...
  TFile* parsedMC = new TFile("SignalExtractionCoarseCS/ClosureYields/CohHe.root");
  ...
  for (Int_t iCosThetaBins5 = 4; iCosThetaBins5 < 20; iCosThetaBins5++) {
    ...
    TFile f(Form("SignalExtractionCoarseCS/ClosureYields/MonteCarloYieldsHe_%d.root", iCosThetaBins5),   "recreate");
...
void DrawExample(){
  new TCanvas;
  new TCanvas;
  TFile* f = new TFile("SignalExtractionCoarseCS/ClosureYields/CohHe.root");
  ...
  gPad->SaveAs("SignalExtractionCoarseCS/ClosureYields/reconstructed-MC.pdf", "recreate");
}
//______________________________________________
void DrawResolution(){
  new TCanvas;
  ...
  gPad->SaveAs("SignalExtractionCoarseCS/ClosureYields/resolution-phi.pdf", "recreate");
}
//______________________________________________
void DrawAxE(){
  new TCanvas;
  ...
  latex10->DrawLatex(0.31,0.94,"This thesis, Pb-Pb #sqrt{#it{s}_{NN}} = 5.02 TeV");
  latex10->DrawLatex(0.2,0.80, "ALICE Simulation, LHC18l7" );
  gPad->SaveAs("SignalExtractionCoarseCS/ClosureYields/costheta.pdf", "recreate");
