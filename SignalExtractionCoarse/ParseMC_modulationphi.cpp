...
// ParseMC_modulationphi(1., 100, 0.)
  ...
  TFile *SavingFile = new TFile("SignalExtractionCoarse/ModulationYields/CohHe_phimodulation.root", "RECREATE");
  for (Int_t i = 0; i < 24; i++) {
    for (Int_t j = 0; j < 24; j++) {
      InvMassH[i][j]->Write();
    }
  ...
  TFile* parsedMC = new TFile("SignalExtractionCoarse/ModulationYields/CohHe_phimodulation.root");
  ...
  for (Int_t iCosThetaBins = 4; iCosThetaBins < 20; iCosThetaBins++) {
    ...
    TFile f(Form("SignalExtractionCoarse/ModulationYields/MonteCarloYieldsHe_phimodulation_%d.root", iCosThetaBins),   "recreate");
