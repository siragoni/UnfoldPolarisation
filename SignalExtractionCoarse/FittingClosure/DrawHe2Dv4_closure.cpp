...
void PolarisationHeMinuit2D(Int_t FitRangeMode = 0, Int_t Iterations = 1, Int_t StripeMode = 0 ){
  ...
  for (Int_t i = 4; i < 20; i++) {
    f[i]         = new TFile(Form("SignalExtractionCoarse/UnfoldingClosure/UnfoldedClosureHe_%d.root", i));
    h[i]         = (TH1F*) f[i]->Get("response;3"); // actual
  }
  ...
  for (Int_t i = 0; i < 600; i++) {
    if (UnlikeSignDimuon2->GetBinContent(i+1) > 0.0000000001){
      ...
      UnlikeSignDimuon->SetBinContent(j, UnlikeSignDimuon2->GetBinContent(i+1)-1.);
      j++;
    }
  }
  UnlikeSignDimuon->GetXaxis()->SetTitle("Bin number");
  UnlikeSignDimuon->GetYaxis()->SetTitle("Ratio of refolded to REC ");
  UnlikeSignDimuon->GetYaxis()->SetRangeUser(-1.e-16, 1.e-16);
  ...
  gPad->SaveAs(Form("SignalExtractionCoarse/FittingClosure/residual-refolded-data-%d.pdf", Iterations), "recreate");
  ...
  histo2->SetTitle( "; #it{#varphi} ;cos(#it{#theta})" );
    ...
    const Double_t min = histo2->GetMaximum()*0.8;
    const Double_t max = histo2->GetMaximum()*1.3;

    const Int_t nLevels = 20;
    ...
    histo2->GetZaxis()->SetRangeUser(600000, 900000); // ... set the range ...
  ...
  latex->DrawLatex(0.15,0.79,Form("N = %0.1f #pm %0.1f ", Normalisation,    NormalisationErr ));
  gPad->SaveAs(Form("SignalExtractionCoarse/FittingClosure/2Dmaps-data-%d.pdf", Iterations), "recreate");
  ...
  histo2->Fit("Model", "R");
  gPad->SaveAs(Form("SignalExtractionCoarse/FittingClosure/3D-data-%d.pdf", Iterations), "recreate");
    ...
    TH1F* pullDistrib  = new TH1F("pullDistrib", "pullDistrib", 100, -10, 10);
    TH1F* resDistrib   = new TH1F("resDistrib", "resDistrib", 200, -4000, 4000);
    TH1F* errorDistrib = new TH1F("errorDistrib", "errorDistrib", 200, 0, 4000);
    for( Int_t i = 0; i < coordsX.size(); i++ ){
      Double_t TraslatedCosThetaGen = 0.5*(coordsY[i] + 1.)*24.;
      Double_t iCosThetaBins2 = -1;
      Double_t RemainderCosTheta = 0.;
      ...
      pullDistrib ->Fill((values[i] - IntegralFunction(xn,yn))/errors[i]);
      resDistrib  ->Fill(values[i] - IntegralFunction(xn,yn));
      errorDistrib->Fill(errors[i]);
    }
    // printf("LambdaTheta     : %+.7f +- %.7f\n",LambdaTheta,     LambdaThetaErr     );
    // printf("LambdaPhi       : %+.7f +- %.7f\n",LambdaPhi,       LambdaPhiErr       );
    // printf("LambdaThetaPhi  : %+.7f +- %.7f\n",LambdaThetaPhi,  LambdaThetaPhiErr  );
    ...
    new TCanvas;
    BeautifyPad();
    BeautifyHisto(pullDistrib);
    pullDistrib->Rebin(8);
    pullDistrib->GetXaxis()->SetTitle("Pulls");
    pullDistrib->GetYaxis()->SetTitle("Counts [a.u.]");
    // pullDistrib->GetYaxis()->SetRangeUser(UnlikeSignDimuon->GetMaximum()*(0.999),UnlikeSignDimuon->GetMaximum()*(1.001));
    // pullDistrib->GetXaxis()->SetRangeUser(-0.5, 215.5);
    pullDistrib->Draw("");
    TF1* fitfunc = new TF1("fitfunc", "gaus");
    pullDistrib->Fit(fitfunc);
    ...
    latexD->DrawLatex(0.2,0.84,Form("#mu = %0.3f #pm %0.3f", fitfunc->GetParameter(1), fitfunc->GetParError(1) ));
    latexD->DrawLatex(0.2,0.78,Form("#sigma = %0.3f #pm %0.3f", fitfunc->GetParameter(2), fitfunc->GetParError(2) ));
    latexD->DrawLatex(0.2,0.7,"Pull = #frac{Unfolded-Fit}{#DeltaUnfolded}" );
    latexD->DrawLatex(0.2,0.62,"Closure" );
    gPad->SaveAs("SignalExtractionCoarse/FittingClosure/pulls-data.pdf", "recreate");
    ...
    resDistrib->Fit(fitfunc2);
    latexD->DrawLatex(0.31,0.94,"This thesis, Pb-Pb #sqrt{#it{s}_{NN}} = 5.02 TeV");
    latexD->DrawLatex(0.2,0.84,Form("#mu = %0.3f #pm %0.3f", fitfunc2->GetParameter(1), fitfunc2->GetParError(1) ));
    latexD->DrawLatex(0.2,0.78,Form("#sigma = %0.3f #pm %0.3f", fitfunc2->GetParameter(2), fitfunc2->GetParError(2) ));
    latexD->DrawLatex(0.2,0.7,"Residual = Unfolded-Fit" );
    latexD->DrawLatex(0.2,0.62,"Closure" );
    gPad->SaveAs("SignalExtractionCoarse/FittingClosure/residuals-data.pdf", "recreate");
    ...
    new TCanvas;
    BeautifyPad();
    BeautifyHisto(errorDistrib);
    errorDistrib->Rebin(8);
    errorDistrib->GetXaxis()->SetTitle("#DeltaUnfolded");
    errorDistrib->GetYaxis()->SetTitle("Counts [a.u.]");
    errorDistrib->Draw("");
    TF1* fitfunc3 = new TF1("fitfunc3", "gaus");
    errorDistrib->Fit(fitfunc3);
    latexD->DrawLatex(0.31,0.94,"This thesis, Pb-Pb #sqrt{#it{s}_{NN}} = 5.02 TeV");
    latexD->DrawLatex(0.5,0.84,Form("#mu = %0.3f #pm %0.3f", fitfunc3->GetParameter(1), fitfunc3->GetParError(1) ));
    latexD->DrawLatex(0.5,0.78,Form("#sigma = %0.3f #pm %0.3f", fitfunc3->GetParameter(2), fitfunc3->GetParError(2) ));
    latexD->DrawLatex(0.2,0.62,"Closure" );
    gPad->SaveAs("SignalExtractionCoarse/FittingClosure/errors-data.pdf", "recreate");
    // gPad->SaveAs("SignalExtractionCoarse/FittingClosure/residuals-data.pdf", "recreate");
...
void FitGeneratedModulationWithSameBinning(Int_t FitRangeMode = 0 ){
  ...
  TH1F*  h[24];
  for (Int_t i = 4; i < 20; i++) {
    f[i] = new TFile(Form("SignalExtractionCoarse/ClosureYields/MonteCarloYieldsHe_%d.root", i));
  ...
  // gMinuit->SetFCN(FcnForMinimisation);
  gMinuit->SetFCN(FcnForMinimisationWithBins);
  gMinuit->DefineParameter(0, "LambdaTheta",        0.6, 0.1,    -2, 2        );
  gMinuit->DefineParameter(1, "LambdaPhi",           0, 0.1,    -2, 2        );
  gMinuit->DefineParameter(2, "LambdaThetaPhi",      0, 0.1,    -2, 2        );
  gMinuit->DefineParameter(3, "Normalisation",   2420000, 100, 1, 1000000000    );
  gMinuit->Command("SIMPLEX");
  ...
  latex->SetTextSize(0.055);
  latex->DrawLatex(0.7,0.94,"Generated");
  latex->SetTextSize(0.04);
  latex->DrawLatex(0.6,0.84,Form("#lambda_{#theta} = %0.3f #pm %0.3f", LambdaTheta,      LambdaThetaErr ));
  latex->DrawLatex(0.6,0.79,Form("#lambda_{#varphi} = %0.3f #pm %0.3f", LambdaPhi,        LambdaPhiErr ));
  latex->DrawLatex(0.6,0.74,Form("#lambda_{#theta#varphi} = %0.3f #pm %0.3f", LambdaThetaPhi,   LambdaThetaPhiErr ));
  latex->DrawLatex(0.15,0.15,Form("#tilde{#chi^{2}} = %0.3f / %d = %0.3f ", GlobalChi,    80, GlobalChi/((Double_t)80) ));
  latex->DrawLatex(0.15,0.79,Form("N = %0.1f #pm %0.1f ", Normalisation,    NormalisationErr ));

  gPad->SaveAs("SignalExtractionCoarse/FittingClosure/generated-binning-data.pdf",  "RECREATE");
  ...
  Model->Draw("surf same");
      ...
      TH1F* residuals2   = new TH1F("residuals2", "residuals2", 500, -0.5, 499.5);
      TH1F* pullDistrib  = new TH1F("pullDistrib", "pullDistrib", 100, -10, 10);
      TH1F* resDistrib   = new TH1F("resDistrib", "resDistrib", 400, -2000, 2000);
      TH1F* errorDistrib = new TH1F("errorDistrib", "errorDistrib", 100, 0, 2000);
      for( Int_t i = 0; i < coordsX.size(); i++ ){
        Double_t TraslatedCosThetaGen = 0.5*(coordsY[i] + 1.)*24.;
        Double_t iCosThetaBins2 = -1;
        Double_t RemainderCosTheta = 0.;
      ...
      latexD->DrawLatex(0.2,0.78,Form("#sigma = %0.3f #pm %0.3f", fitfunc->GetParameter(2), fitfunc->GetParError(2) ));
      latexD->DrawLatex(0.2,0.7,"Pull = #frac{Unfolded-Fit}{#DeltaUnfolded}" );
      latexD->DrawLatex(0.2,0.62,"Generated" );
      gPad->SaveAs("SignalExtractionCoarse/FittingClosure/pulls-generated.pdf", "recreate");
      ...
      latexD->DrawLatex(0.2,0.7,"Residual = Unfolded-Fit" );
      latexD->DrawLatex(0.2,0.62,"Generated" );
      gPad->SaveAs("SignalExtractionCoarse/FittingClosure/residuals-generated.pdf", "recreate");
      ...
      latexD->DrawLatex(0.5,0.78,Form("#sigma = %0.3f #pm %0.3f", fitfunc3->GetParameter(2), fitfunc3->GetParError(2) ));
      latexD->DrawLatex(0.2,0.62,"Generated" );
      gPad->SaveAs("SignalExtractionCoarse/FittingClosure/errors-generated.pdf", "recreate");


}
//_____________________________________________________________________________
void UnfoldGen(Int_t Iterations = 1){
  ...
  for (Int_t i = 5; i < 19; i++) {
    f[i]  = new TFile(Form("SignalExtractionCoarse/ClosureYields/MonteCarloYieldsHe_%d.root", i));
    f2[i] = new TFile(Form("SignalExtractionCoarse/UnfoldingClosure/UnfoldedClosureHe_%d.root", i));
    h[i]  = (TH1F*) f2[i]->Get("response;3"); // actual
    hg[i] = (TH1F*) f[i]->Get(Form("hg_%d", i));
  }
  // =========================================
  // Drawing Refolded Distribution
  // -----------------------------------------
  ...
  BeautifyPad();
  Double_t ChiSquareTestRef = 0;
  TH1F* unfoldgen = new TH1F("unfoldgen","unfoldgen", 300, -0.5, 299.5);
  BeautifyHisto(unfoldgen);
  Int_t Counter = 1;
  for (Int_t i = 5; i < 19; i++) {
    for (Int_t j = 1; j < 7; j++) {
      ChiSquareTestRef += (h[i]->GetBinContent(j) - hg[i]->GetBinContent(j))*(h[i]->GetBinContent(j) - hg[i]->GetBinContent(j))/hg[i]->GetBinContent(j);
      unfoldgen->SetBinContent(Counter, (h[i]->GetBinContent(j) - hg[i]->GetBinContent(j))/hg[i]->GetBinContent(j) );
      Counter++;
    }
  }
  unfoldgen->GetXaxis()->SetTitle("Bin number");
  unfoldgen->GetYaxis()->SetTitle("(Unfold REC - GEN)/GEN [%]");
  unfoldgen->GetYaxis()->SetRangeUser(-1.2e-14,1.2e-14);
  unfoldgen->GetXaxis()->SetRangeUser(-0.5, 85.5);
  unfoldgen->Draw("");
  TLatex* latex10 = new TLatex();
  latex10->SetTextSize(0.045);
  latex10->SetTextFont(42);
  ...
  latex10->SetNDC();
  latex10->DrawLatex(0.31,0.94,"This thesis, Pb-Pb #sqrt{#it{s}_{NN}} = 5.02 TeV");
  if (Iterations == 1){latex10->DrawLatex(0.31,0.62,"1 iteration");}
  else {latex10->DrawLatex(0.31,0.62,Form("%d iterations", Iterations));}
  latex10->DrawLatex(0.14,0.80,Form("Cumulative spread = #sum (Unfold REC - GEN)^{2}/GEN = %0.02f", ChiSquareTestRef));
  gPad->SaveAs(Form("SignalExtractionCoarse/FittingClosure/residual-unfolded-gen-%d.pdf", Iterations), "recreate");
