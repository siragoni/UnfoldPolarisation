...
void PolarisationHeMinuit2D(Int_t FitRangeMode = 0, Int_t Iterations = 1, Int_t StripeMode = 0 ){
  ...
  Int_t nBins = 1;
  Double_t costhetaprojection[24];
  Double_t costhetaprojectionerror[24];
  for (Int_t iCosThetaBins = 5; iCosThetaBins < 19; iCosThetaBins++) {
    ...
    for (Int_t iPhiBins = 0; iPhiBins < nBins; iPhiBins++) {
      ...
      costhetaprojection[iCosThetaBins]      += h[iCosThetaBins]->GetBinContent(iPhiBins+1);
      costhetaprojectionerror[iCosThetaBins] += h[iCosThetaBins]->GetBinError(iPhiBins+1) * h[iCosThetaBins]->GetBinError(iPhiBins+1);
    }
    costhetaprojectionerror[iCosThetaBins] = TMath::Sqrt(costhetaprojectionerror[iCosThetaBins]);
  }
  ...
  // =========================================
  // Drawing projection in CosTheta
  // -----------------------------------------
  new TCanvas;
  BeautifyPad();
  TH1F* ProjectionCosTheta   = new TH1F("ProjectionCosTheta", "ProjectionCosTheta", 24, -1, 1);
  for (Int_t i = 0; i < 24; i++) {
    ProjectionCosTheta->SetBinContent(i+1, costhetaprojection[i]);
    ProjectionCosTheta->SetBinError(i+1, costhetaprojectionerror[i]);
  }
  BeautifyHisto(ProjectionCosTheta);
  ProjectionCosTheta->GetXaxis()->SetTitle("cos#theta");
  ProjectionCosTheta->GetYaxis()->SetTitle("dN/dcos#theta ");
  ProjectionCosTheta->GetYaxis()->SetRangeUser(0.,15000);
  // ProjectionCosTheta->GetXaxis()->SetRangeUser(-0.5, 85.5);
  ProjectionCosTheta->Draw("");
  latex10->DrawLatex(0.31,0.94,"This thesis, Pb-Pb #sqrt{#it{s}_{NN}} = 5.02 TeV");
  if (Iterations == 1){latex5->DrawLatex(0.4,0.80,"1 iteration");}
  else {latex10->DrawLatex(0.4,0.80,Form("%d iterations", Iterations));}
  latex10->DrawLatex(0.2,0.80,"Projection");
  gPad->SaveAs(Form("SignalExtractionCoarse/Fitting/projection-data-%d.pdf", Iterations), "recreate");
  ...
  histo2->Fit("Model", "R");
  gPad->SaveAs(Form("SignalExtractionCoarse/Fitting/3D-data-%d.pdf", Iterations), "recreate");
  ...
  TH1F* resDistrib   = new TH1F("resDistrib", "resDistrib", 100, -1000, 1000);
  TH1F* errorDistrib = new TH1F("errorDistrib", "errorDistrib", 50, 0, 1000);
  ...
  gPad->SaveAs("SignalExtractionCoarse/Fitting/pulls-data.pdf", "recreate");
  ...
  gPad->SaveAs("SignalExtractionCoarse/Fitting/residuals-data.pdf", "recreate");
  ...
  // latexD->DrawLatex(0.2,0.7,"Residual = Unfolded-Fit" );
  gPad->SaveAs("SignalExtractionCoarse/Fitting/errors-data.pdf", "recreate");
